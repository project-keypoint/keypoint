...
<div id="pagination">
	...
	<a href="${pageContext.request.contextPath }/material/purchaseMaterialList2?materialCode=${search.materialCode}&materialName=${search.materialName}&pageNum=${pageDTO.currentPage-1}"><</a>
	</c:if>

<!-- 10페이지 이전 -->
	 <c:if test="${pageDTO.startPage > pageDTO.pageBlock}">
	<a href="${pageContext.request.contextPath }/material/purchaseMaterialList2?materialCode=${search.materialCode}&materialName=${search.materialName}&pageNum=${pageDTO.startPage-PageDTO.pageBlock}"><<</a>
	</c:if>
	
	<c:forEach var="i" begin="${pageDTO.startPage }" end="${pageDTO.endPage }" step="1">
	<a href="${pageContext.request.contextPath }/material/purchaseMaterialList2?materialCode=${search.materialCode}&materialName=${search.materialName}&pageNum=${i}" <c:if test="${pageDTO.pageNum eq i}">class="active"</c:if>>${i}</a> 
	</c:forEach>
