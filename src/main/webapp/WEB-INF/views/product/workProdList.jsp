	...
	<div class="center">
		<div id="pagination">
			...
			<c:if test="${pageDTO.currentPage > 1}">
				<a
					href="${pageContext.request.contextPath }/product/workProdList?productCode=${search.productCode}&productName=${search.productName}&pageNum=${pageDTO.currentPage-1}"><</a>
			</c:if>

			<!-- 10페이지 이전 -->
			<c:if test="${pageDTO.startPage > pageDTO.pageBlock}">
				<a
					href="${pageContext.request.contextPath }/product/workProdList?productCode=${search.productCode}&productName=${search.productName}&pageNum=${pageDTO.startPage-PageDTO.pageBlock}"><<</a>
			</c:if>

			<c:forEach var="i" begin="${pageDTO.startPage }"
				end="${pageDTO.endPage }" step="1">
				<a
					href="${pageContext.request.contextPath }/product/workProdList?productCode=${search.productCode}&productName=${search.productName}&pageNum=${i}"
